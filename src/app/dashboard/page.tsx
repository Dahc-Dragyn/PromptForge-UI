...
const DashboardContent = () => {
  ...
  const visibleTemplates = useMemo(() => {
    ...
    return showArchivedTemplates ? sorted : sorted.filter(t => !t.is_archived);
  ...
  const handleDeletePrompt = (promptId: string) => {
    if (window.confirm('Are you sure you want to delete this prompt?')) {
      handleAction(deletePrompt(promptId), { loading: 'Deleting...', success: 'Prompt deleted.', error: 'Failed to delete.' });
    }
  };
  const handleRate = (promptId: string, versionNumber: number, rating: number) => {
    handleAction(ratePrompt(promptId, versionNumber, rating), { loading: 'Submitting...', success: 'Rating submitted!', error: 'Failed to submit rating.' })
      ...
      <div className="p-4 sm:p-8 bg-gray-900 min-h-screen text-white">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <TopPromptsWidget prompts={prompts ?? []} loading={promptsLoading} isError={promptsError} onCopy={handleCopyText} onArchive={handleArchivePrompt} copiedPromptId={copiedPromptId} />
            <div className="lg:col-span-2">
              <RecentActivityWidget activities={activities ?? []} loading={activityLoading} isError={activityError} prompts={prompts ?? []} onCopy={handleCopyText} onArchive={handleArchivePrompt} onDelete={handleDeletePrompt} copiedPromptId={copiedPromptId} />
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col min-h-0">
              ...
              <div className="flex items-center gap-4">
                ...
                <label className="flex items-center cursor-pointer">
                  ...
                  <div className="relative">
                    <input type="checkbox" checked={showArchivedTemplates} onChange={() => setShowArchivedTemplates(!showArchivedTemplates)} className="sr-only" />
                    <div className={`block w-14 h-8 rounded-full transition-colors ${showArchivedTemplates ? 'bg-green-500' : 'bg-gray-700'}`}></div>
                    <div className={`dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform ${showArchivedTemplates ? 'translate-x-full' : ''}`}></div>
                  </div>
                </label>
              </div>
            </div>
            <div className="bg-gray-800 p-4 rounded-lg flex-grow overflow-y-auto">
              ...
              {visibleTemplates.map((template) => (
                <div key={template.id} className={`p-4 bg-gray-700/50 rounded-lg mb-4`}>
                  <Link href={`/templates/${template.id}`} className={`font-semibold mb-1 block hover:underline ${template.is_archived ? 'text-gray-500 line-through' : 'text-blue-400'}`}>{template.name}</Link>
                  <p className="text-sm text-gray-400 line-clamp-2 mb-3">{template.description}</p>
                  <div className="flex items-center justify-between pt-3 border-t border-gray-700/50">
                    <div className="flex items-center gap-x-2">
                      <Link href={`/templates/${template.id}`} className="px-3 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700">View</Link>
                      <button onClick={() => handleCopyTemplate(template.id)} className="px-3 py-1 text-xs bg-gray-600 text-white rounded hover:bg-gray-500">Copy</button>
                      <button onClick={() => handleArchiveTemplate(template.id, !template.is_archived)} className="p-1.5 text-xs bg-yellow-600 text-white rounded hover:bg-yellow-700" title={template.is_archived ? 'Unarchive' : 'Archive'}>{template.is_archived ? <ArrowUturnLeftIcon className="h-4 w-4" /> : <ArchiveBoxIcon className="h-4 w-4" />}</button>
                      <button onClick={() => handleDeleteTemplate(template.id)} className="p-1.5 text-xs bg-red-600 text-white rounded hover:bg-red-700" title="Delete"><TrashIcon className="h-4 w-4" /></button>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-gray-400">Send to:</span>
                      <button onClick={() => handleSendTemplateToLlm(template.content, 'ChatGPT')} className="px-2 py-1 text-xs bg-[#10a37f] text-white rounded hover:bg-[#0daaa5]">ChatGPT</button>
                      <button onClick={() => handleSendTemplateToLlm(template.content, 'Gemini')} className="px-2 py-1 text-xs bg-[#4e85ff] text-white rounded hover:bg-[#588dff]">Gemini</button>
                      <button onClick={() => handleSendTemplateToLlm(template.content, 'Grok')} className="px-2 py-1 text-xs bg-gray-600 text-white rounded hover:bg-gray-500">Grok</button>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="flex flex-col min-h-0">
            ...
            <div className="bg-gray-800 p-4 rounded-lg flex-grow overflow-y-auto">
              ...
              {visiblePrompts.map((prompt) => (
                <div key={prompt.id} className={`p-4 bg-gray-700/50 rounded-lg mb-4`}>
                  <div className="flex justify-between items-start">
                    <Link href={`/prompts/${prompt.id}`} className={`font-semibold mb-1 block hover:underline ${prompt.is_archived ? 'text-gray-500 line-through' : 'text-indigo-400'}`}>{prompt.name}</Link>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-gray-700/50">
                    <div className="flex items-center flex-wrap gap-x-4 gap-y-2">
                      ...
                      <div className="flex items-center gap-x-2">
                        ...
                        <button onClick={() => handleArchivePrompt(prompt.id, !prompt.is_archived)} className="p-1.5 text-xs bg-yellow-600 text-white rounded hover:bg-yellow-700" title={prompt.is_archived ? 'Unarchive' : 'Archive'}>{prompt.is_archived ? <ArrowUturnLeftIcon className="h-4 w-4" /> : <ArchiveBoxIcon className="h-4 w-4" />}</button>
                        <button onClick={() => handleDeletePrompt(prompt.id)} className="p-1.5 text-xs bg-red-600 text-white rounded hover:bg-red-700" title="Delete"><TrashIcon className="h-4 w-4" /></button>
                      </div>
                    </div>

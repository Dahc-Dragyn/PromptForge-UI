...
// --- FIX: Import 'doc' and 'deleteDoc' from firestore ---
...
3. Your entire response MUST be ONLY the raw JSON object. Do not include markdown.`;

        try {
            const response = await authenticatedFetch(`${API_BASE_URL}/execute`, {
                method: 'POST',
                body: JSON.stringify({
            ...
            let jsonString = data.final_text;
    ...
    const handleTestImprovement = (newPrompt: string) => {
        setPromptText(newPrompt);
        handleApiCall('diagnose', { prompt_text: newPrompt });
        toast.success("Testing suggested improvement...");
    };
    ...
    const handleOpenSaveModal = async () => {
        ...
        try {
            ...
            const [nameRes, descRes] = await Promise.all([
                authenticatedFetch(`${API_BASE_URL}/execute`, { method: 'POST', body: JSON.stringify({ prompt_text: nameMetaPrompt, model, variables: {} }) }),
                authenticatedFetch(`${API_BASE_URL}/execute`, { method: 'POST', body: JSON.stringify({ prompt_text: descMetaPrompt, model, variables: {} }) })
            ]);
    ...
    const handleSavePrompt = async (e: React.FormEvent) => {
        ...
        let docRef; // Define docRef here to access it in the catch block

        try {
            docRef = await addDoc(collection(db, 'prompts'), {
                name: newPromptName,
            ...
            const versionResponse = await authenticatedFetch(versionsUrl, {
                method: 'POST',
                body: JSON.stringify({
            ...
            if (!versionResponse.ok) {
                 ...
                 throw new Error(errorData.detail || 'Failed to save the initial prompt version.');
            }
            ...
            // If something fails, delete the Firestore document that was created
            if (docRef) {
                // --- FIX: Use the correct deleteDoc function ---
            ...
            <div className="min-h-screen bg-gray-900 text-white p-4 sm:p-8">
                <div className="max-w-4xl mx-auto">
                    ...
                    {output && (
                        <div className="mt-8">
                            {outputType === 'diagnose' ? (
                                <DiagnoseResult data={output} onTestImprovement={handleTestImprovement} />

...
// *** FIX 1 (THE CRASH): Use a named import { SendToLlm } to match the export ***
          ...
          <li key={prompt.id} className={`p-3 rounded-lg transition-all ${prompt.is_archived ? 'bg-gray-700/50 opacity-60' : 'bg-gray-700/80 hover:bg-gray-700/100'}`}>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 pt-3 mt-2 border-t border-gray-600/50">
                <div className="flex items-center gap-2">
                   ...
                   {/* *** FIX 2 (THE VIEW BUTTON): Added the missing "View" link *** */}
                   <Link href={`/prompts/${prompt.id}`} className="px-3 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700">
                      View
                   </Link>
                </div>
                {/* This line will now work because of Fix 1 */}

...
// import { useTemplateMutations } from '@/hooks/usePromptTemplates'; // Removed unused import
import { PromptTemplate } from '@/types/template';
import toast from 'react-hot-toast';

// Define the shape of the data the form will submit
// NOTE: Assuming the missing field required by the compiler is 'version' and new templates start at 1.
...
// Utility function to safely add a tag to the comma-separated string
const addTag = (currentTags: string, newTag: string): string => {
    // 1. Tokenize, clean, and filter out empty strings
    const tagsArray = currentTags.split(',').map(tag => tag.trim()).filter(Boolean);
    // 2. Use a Set for guaranteed deduplication
    const uniqueTags = new Set(tagsArray);
    uniqueTags.add(newTag.toLowerCase());
    // 3. Convert back to comma-separated string
...
const TemplateForm = ({ onSubmit, onCancel, initialData = {} }: TemplateFormProps) => {
    ...
    const [templateType, setTemplateType] = useState(initialData.tags?.includes('persona') ? 'persona' : initialData.tags?.includes('task') ? 'task' : 'general');
    const [tags, setTags] = useState((initialData.tags || []).join(', '));
    const [isSubmitting, setIsSubmitting] = useState(false);

    // --- NEW: Handlers for the tag buttons ---
    const handleAddPersona = () => {
        setTags(prevTags => addTag(prevTags, 'persona'));
        setTemplateType('persona');
    };

    const handleAddTask = () => {
        setTags(prevTags => addTag(prevTags, 'task'));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // --- FIX: Added 'version: 1' to satisfy TemplateFormData type requirement ---
        const newTemplate: TemplateFormData = {
            name,
            description,
            content,
            tags: finalTags,
            version: 1, // Assumed default version for new creation
        };
    ...
    return (
        <form onSubmit={handleSubmit} className="p-1 space-y-4">
            ...
            {/* --- NEW: Tags Input with Helper Buttons --- */}
            <div>
                ...
                <div className="mt-2 flex gap-2 items-center">
                    <button type="button" onClick={handleAddPersona} className="text-xs px-2 py-1 bg-indigo-600/70 text-white rounded hover:bg-indigo-700">
                        + Persona
                    </button>
                    <button type="button" onClick={handleAddTask} className="text-xs px-2 py-1 bg-indigo-600/70 text-white rounded hover:bg-indigo-700">
                        + Task
                    </button>
                    <span className="text-xs text-gray-500 italic ml-2">
                        Add tags required for Manual Composer grouping.
                    </span>
                </div>
            </div>
            {/* --- END NEW --- */}
